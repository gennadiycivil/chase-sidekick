...
- **Slack-Ready Messages**: When Gennadiy asks for a Slack message, provide Slack-ready cut-and-paste text by default. Do not wrap it in a code fence, add preamble, or include commentary unless he explicitly asks for alternatives or explanation.
...
- Treat Dropbox Paper / Dropbox shared docs as one of the top sources, usually #1 or #2 alongside Slack.
- Search Dropbox Paper / Dropbox links early, before settling on Confluence, Google Drive, or JIRA-derived candidates.
- For references mentioned to a person, search that person's Slack DM/channel history for Dropbox Paper links (`dropbox.com/scl`, `.paper`, `paper`, and "this doc"/"this one"/"platform doc" style phrases), not only document title text.
- Do not assume Confluence is the source of truth for strategy or planning docs unless the user explicitly says Confluence or provides a Confluence URL.
